...
$selected_currency=$getdataResult['curr'];
...
$insertQuery=mysqli_query($con,"INSERT INTO `tbl_userresult3` (`userid`, `periodid`, `type`, `value`, `amount`, `openprice`, `tab`, `paidamount`, `fee`, `status`, `createdate`,`curr`) VALUES ('".$userid."', '".$userperiodid."', '".$type."', '".$value."','".$amount."', '".$openprice."', '".$tab."', '".$paidamount."', '".$fee."', '".$status."','".$date."','".$selected_currency."')");


   if($selected_currency == 'trx'){
...
$sqlwallet= mysqli_query($con,"UPDATE `tbl_wallet` SET `amount` = '".$finalbalancecredit."' WHERE `userid`= '".$userid."'");
   ...
   if($status=='success'){
       ...
       $paidamount1 = $paidamount / 11.04;

	
$sql= mysqli_query($con,"INSERT INTO `tbl_order`(`userid`,`transactionid`,`amount`,`status`,`createdate`) VALUES ('".$userid."','".$userperiodid."','".$paidamount1."','1','".$date."')");
@$orderid=mysqli_insert_id($con);

$sql3= mysqli_query($con,"INSERT INTO `tbl_walletsummery`(`userid`,`orderid`,`amount`,`type`,`actiontype`,`createdate`) VALUES ('".$userid."','".$orderid."','".$paidamount1."','credit','win','".$date."')");

$selectwallet=mysqli_query($con,"select `amountusdt` from `tbl_wallet` where `userid`='".$userid."'");
$walletResult=mysqli_fetch_array($selectwallet);
 	
$finalbalancecredit=$walletResult['amountusdt']+$paidamount1;	
