...
<div style="background:#FBEDF3;border-radius:18px;" class="" id="paritycontainer">
   <table class="table text-center" id="parityt">
      ...
      <tbody>
         <?php
            $parityrecordQuery=mysqli_query($con,"select * from `tbl_result2` where `tabtype`='parity' order by id desc limit 480");
...
<div style="background:#FBEDF3;border-radius:18px; padding-bottom:48px;">
   <table style="background:#fff" class="table " id="myrecordparityt">
      <tbody>
         <?php
            $userResultQuery=mysqli_query($con,"select *,(select `result` from `tbl_result2` where `periodid`=`tbl_userresult2`.`periodid` and `tabtype`=`tbl_userresult2`.`tab` and `curr`=`tbl_userresult2`.`curr`)as resultnumber,(select `color` from `tbl_result2` where `periodid`=`tbl_userresult2`.`periodid` and `tabtype`=`tbl_userresult2`.`tab`)as resultcolor from `tbl_userresult2` where `userid`='".$userid."' and `tab`='parity'  order by id desc limit 10");
...
?></span></div>
   ...
   <div style="float:right; margin:10px;">
      <span style="padding:7px 10px; border:1px solid black;color:black" ><a href="order2.php" style="color:black">More <i class="fa fa-angle-right" aria-hidden="true"></i></a></span>
   </div>
</div>
