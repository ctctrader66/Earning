...
if(isset($_POST['submit'])=='Submit'){
	...
	$reward=($_POST['reward']);
		$note=($_POST['note']);
	  $reward_currency = $_POST['reward_currency']; 

	if($reward_currency == 'trx'){
	    $sql= "INSERT INTO `tbl_reward`(`userid`,`amount`,`note`) VALUES ('".$userid."','".$reward."','".$note."')";
	}else{
	     $sql= "INSERT INTO `tbl_reward`(`userid`,`amountusdt`,`note`) VALUES ('".$userid."','".$reward."','".$note."')";
	}
...
$sql3= mysqli_query($con,"INSERT INTO `tbl_walletsummery`(`userid`,`orderid`,`amount`,`type`,`actiontype`,`createdate`) VALUES ('".$userid."','".$orderid."','".$reward."','credit','$note','".$today."')");
...
$sqlwallet= mysqli_query($con,"UPDATE `tbl_wallet` SET `amount` = '$finalbalanceCredit' WHERE `userid`= '$userid'");
	    ...
	    $walletAvailablebalance=wallet($con,'amountusdt',$userid);	
$finalbalanceCredit=$walletAvailablebalance+$reward;	
...
<div class="col-sm-3">
  <div class="form-group">
    <label>Reward Currency</label><br>
    <input type="radio" name="reward_currency" value="trx" required> TRX
    <input type="radio" name="reward_currency" value="usdt"> USDT
  </div>
</div>
             ...
             <select id="note" name="note" class="form-control">
                                    ...
                                    <option value="refer_comission">Refer Comission</option>
                                    <option value="reward">Reward</option>
                                    <option value="recharge_bonus">Recharge Bonus</option>
